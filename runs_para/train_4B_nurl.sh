...
export EXP_NAME=${EXP_NAME:-qwen3_4B_nurl_stage2_hard1024_resp8192_prompt2048}
# Stage2 should load Stage1 GRPO checkpoint (HF format) by default.
...
bash /hyk/algorithm_new/qinghua/yueyang/verl/exp_scripts/train_4B_v7_switchable.sh \
  data.ts_version=nurl \
  data.curri_method='teacher_student' \
  data.train_files=/hyk/algorithm_new/qinghua/yueyang/verl/data/nurl/hard_1024_stage2_nurl.parquet \
  data.val_files=/hyk/algorithm_new/qinghua/yueyang/verl/data/valid_final.parquet \
  data.max_prompt_length=2048 \
  data.max_response_length=8192 \
  ++data.all_problems_are_hard=true \
  +data.nurl_hints_jsonl_path=/hyk/algorithm_new/qinghua/yueyang/verl/data/nurl/hard_1024_with_abstract.jsonl \
